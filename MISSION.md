...
- _2026-07-18_ - **Kill criteria for the ai-probability Polymarket strategy — APPROVED by Richard in chat ("Yes, add the kill criteria to MISSION.md").** Evaluation window: 30 days from today (through 2026-08-17) or until 100 trades settle under the repaired pipeline, whichever comes first. The clean cohort starts 2026-07-18 (post-R3: honest best-level fills, real pre-fill drift re-validation, equity-anchored sizing); the 54 trades settled today are the polluted pre-fix cohort and do NOT count toward the clean sample. KILL the ai-probability strategy if, at window close, BOTH hold: (1) clean-cohort realized P&L is negative, and (2) calibration shows the model's probability estimates are no better than market prices (Brier score vs market-price baseline from `npm run poly:settled:calibration`). Kill means: halt the strategy and stop LLM eval spend; the infrastructure (scanner, gates, broker, resolution pipeline) is explicitly NOT killed and remains available for a replacement strategy or equities-only operation. Goalposts fixed now; neither party relitigates mid-window. This entry does not enable real money, change any monetary parameter, or affect Box 7.
