...
function renderEvidencePath(evidence) {
  ...
  const pipelineEl = document.getElementById('evidence-poly-pipeline');
  if (pipelineEl) {
    pipelineEl.textContent = (poly.potentialSettledTrades ?? ((poly.settledTrades ?? 0) + (poly.openTrades ?? 0))) + '/' + (poly.targetSettledTrades ?? 50);
    pipelineEl.style.color = poly.openPipelineCanReachTarget ? '#6ee7b7' : '#fbbf24';
  }
